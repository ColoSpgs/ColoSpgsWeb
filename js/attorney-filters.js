...
class AttorneyFilters {
  ...
  /**
   * Fetch unique specialties from attorneys
   */
  async fetchSpecialties() {
    try {
      const { data, error } = await supabase
        .from('profiles')
        .select('legal_specialties')
        .contains('type', ['attorney'])
        .not('legal_specialties', 'is', null)
      ...
      this.specialties = [
        ...new Set(
          data
            .map((profile) => profile.legal_specialties)
            .flat()
  ...
  handleSpecialtySelection(specialty) {
    ...
    // Dispatch only filterChange event with the selected specialty
...
document.addEventListener('DOMContentLoaded', () => {
  window.attorneyFilters = new AttorneyFilters()
})
